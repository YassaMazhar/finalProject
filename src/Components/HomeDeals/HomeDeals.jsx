...
export default function HomeDeals() {
  let [productDeals, setProductsDeals] = useState(null);
  ...
  async function getDeals() {
    try {
      ...
      if (response.success) {
        setProductsDeals(response.data.data);
        setIsLoading(false);
      }
    } catch (error) {
      setIsLoading(false);
  ...
  let deals = productDeals?.filter((Product)=> Product.priceAfterDiscount).slice(0,5) || []
      ...
      <section className="py-8 ">
        <div className="container space-y-4">
          <div className="top section ">
            <div>
              <h3 className="text-2xl font-bold mb-2 capitalize">Deals of the Day</h3>
